                ...
                <table class='table table-striped'>
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>Item Id</th><th>Name</th><th>Model</th><th>Brand</th><th>Type</th><th>Description</th><th>Quantity</th><th>Unit</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        @forelse($items as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->itemId }}</td><td>{{ $item->name }}</td><td>{{ $item->model }}</td><td>{{ $item->brand }}</td>
                                <td><b class="text-success">{{ $item->types->name ?? "no data" }}</b></td>
                                <td>{{ $item->description }}</td><td>{{ $item->quantity }}</td><td>{{ $item->unit }}</td>
                                <td>
                                    <a href='{{ route('items.restore', $item->id) }}'><i class='fas fa-recycle text-info'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-restore').click(function () {
                ...
                $.post('/items-restore-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    console.log(res)
